...
class County extends Component {
    ...
    render(){
       ...
       highlight.forEach(d=>highlighto[d.toLowerCase()]=1);
        ...
        const _textData = geographies.map(d=>{
            const p = geoPath().projection(projection).centroid(d);
            return [(p[0]+this.zoom.x)*this.zoom.k,(p[1]+this.zoom.y)*this.zoom.k]
        });
        const voronoi = d3.Delaunay.from(_textData).voronoi([-1, -1, map_width + 1, map_height + 1])
        const textData = geographies.map((d,i)=>{
            const cell = voronoi.cellPolygon(i);
            if(cell) {
                const [cx, cy] = d3.polygonCentroid(cell);
                const [x, y] = _textData[i];
                const angle = (Math.round(Math.atan2(cy - y, cx - x) / Math.PI * 2) + 4) % 4;
                const props = angle === 0 ? {"textAnchor": "start", "dy": "0.35em", "x": 6}
                    : angle === 3 ? {"textAnchor": "middle", "x": -6}
                        : angle === 1 ? {"textAnchor": "middle", "dy": "0.71em", "x": 6}
                            : {"textAnchor": "end", "dy": "0.35em", "x": -6};
                return {
                    x,
                    y,
                    props,
                    text: geographies[i].properties.NAME,
                    display: -d3.polygonArea(cell) > 1000 ? null : "none"
                }
            }
            return {
                x:0,
                ...
                <g className="label">
                    {
                        textData.map((d, i) => (
                            <text
                                ...
                                fill={(d.text.toLowerCase() === target) ? 'black' : (highlighto[d.text.toLowerCase()] ? (this.state.Name === d.text ? 'black' : 'black') : `gray`)}
                                style={{pointerEvents: 'none',display:d.display}}
                                {...d.props}
                            >
                                {d.text.toUpperCase()}
                            </text>
