...
import {fromDataAsync} from "xlsx-populate/browser/xlsx-populate";
...
import { districts, districtCode, counties, str2date, date2str } from "./ulti.jsx";
...
class CRCP extends Component {
  ...
  onLoadInput = (event) => {
    ...
    if (event.target.files[0]) {
      fromDataAsync(event.target.files[0]).then(function(workbook) {
  ...
  onSaveInput = () => {
    ...
    fetch(excelOutputFile).then(res => res.arrayBuffer()).then(file => {
      fromDataAsync(file).then(function(workbook) {
